...
DynamicLinearAllocator::DynamicLinearAllocator(Allocator& backing_allocator, size_t block_size, uint8_t block_alignment)
...
void* DynamicLinearAllocator::allocate(size_t size, uint8_t alignment)
{
	...
	if(_current_block->used_memory + adjustment + size > _current_block->size)
	{
		...
		adjustment =alignForwardAdjustment(_current_pos, alignment);
	}
	...
	_current_pos = add(_current_pos, total_size);
...
void DynamicLinearAllocator::rewind(void* p)
{
	...
	while(p <= _current_block + 1 || p > add(_current_block->start, _current_block->size))
	{
	...
	ASSERT(p > _current_block + 1 && p <= add(_current_block->start, _current_block->used_memory));

	size_t k = _current_block->used_memory - ((uintptr_t)p - (uintptr_t)_current_block->start);
...
_used_memory -= _current_block->used_memory - ((uintptr_t)p - (uintptr_t)_current_block->start);
...
bool DynamicLinearAllocator::allocateNewBlock(size_t size, uint8_t alignment)
{
	...
	uint8_t new_block_desc_adjustment = alignForwardAdjustment(new_block, __alignof(BlockDesc));

	BlockDesc* new_block_desc = (BlockDesc*)add(new_block, new_block_desc_adjustment);
