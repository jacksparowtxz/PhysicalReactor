...
SmallBlockAllocator::SmallBlockAllocator(Allocator& backing_allocator, uint32_t block_size, uint32_t block_alignment)
	...
	_block_size(block_size), _block_alignment(block_alignment)
{
	...
	static const uint8_t MIN_ALIGNMENT = 8;
...
void* SmallBlockAllocator::allocate(size_t size, uint8_t alignment)
{
	...
	while(free_block != nullptr)
	{
		...
		uint8_t adjustment = alignForwardAdjustmentWithHeader<AllocationHeader>(free_block, alignment);
	...
	if(best_fit == nullptr)
	{
		...
		best_fit_adjustment = alignForwardAdjustmentWithHeader<AllocationHeader>(best_fit, alignment);
...
SmallBlockAllocator::NewBlock SmallBlockAllocator::allocateNewBlock()
{
	...
	else
	{
		...
		while(free_block != nullptr)
		{
			if((uintptr_t)free_block >= (uintptr_t)new_block.block)
				break;
