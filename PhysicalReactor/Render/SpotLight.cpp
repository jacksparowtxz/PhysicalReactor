...
SpotLight::SpotLight() :color({ 1.0f,1.0f,1.0f,1.0f }), OuterConeAngle(0.0f), Position({ 1.0f,1.0f,1.0f }), Intensity(10.f), Rotaion({0.f,0.f,0.f}), InnerConeAngle(0.0f),  attenuationradius(0.0f), sourceradius(0.0f), softsourceradius(0.f),sourcelengh(0.0f)
...
SpotLight::SpotLight(XMFLOAT3 position, XMFLOAT3 rotaion) :Position(position), Rotaion(rotaion)
{
	...
	sourcelengh = 0.0f;
	lightAngleScale=1.0f/fmax(0.001f,(std::cos(InnerConeAngle) - std::cos(OuterConeAngle)));
	lightAngleOffset=-OuterConeAngle * lightAngleScale;
}
